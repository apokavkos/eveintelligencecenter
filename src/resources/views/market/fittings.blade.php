    ...
    <div class="row">
        <div class="col-md-4">
            ...
            <div class="card card-primary card-outline">
                ...
                <div class="card-body">
                    <form id="fit-paste-form" action="{{ route('eveintelligencecenter::market.fittings') }}" method="POST">
                        @csrf
                                    ...
                                    <tr class="export-row" data-label="{{ strtolower($export->label) }}">
                                        ...
                                        <td class="text-right">
                                            <div class="btn-group">
                                                ...
                                                <form action="{{ route('eveintelligencecenter::market.exports.delete', $export->id) }}" method="POST" onsubmit="return confirm('Delete list?')" style="display:inline;">
                                                    @csrf @method('DELETE')
                                        ...
                                        <tr class="group-{{ $group->id }}-content-wrapper">
                                            <td colspan="4" class="p-0 border-0">
                                                <table class="table table-sm mb-0 w-100" style="table-layout: fixed;">
                                                    @include('eveintelligencecenter::market.partials.fit_row', ['fit' => $fit])
                                                </table>
                                            </td>
                                        </tr>
                                    @endforeach
    ...
    <div class="modal fade" id="saveFitModal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form id="save-fit-modal-form" action="{{ route('eveintelligencecenter::market.fittings.save') }}" method="POST">
                    @csrf
...
<script>
    ...
    $(function () {
        ...
        $('#hub-search-select').select2({
            ajax: {
                url: '{{ route("eveintelligencecenter::market.hubs.search") }}',
                dataType: 'json',
                delay: 250,
        ...
        $('#confirm-add-hub').on('click', function() {
            ...
            $.post('{{ route("eveintelligencecenter::market.hubs.add") }}', {
                _token: '{{ csrf_token() }}',
                hub_id: data.id,
                name: data.text,
                type: data.type
    ...
    function createGroupFromSelected() {
        ...
        if (!name) return;
        $.post('{{ route("eveintelligencecenter::market.groups.save") }}', { _token: '{{ csrf_token() }}', name: name, fit_ids: ids }, function() { location.reload(); });
    }

    function editGroupLabel(group) {
        var newName = prompt('Rename group:', group.name);
        if (!newName || newName === group.name) return;
        $.post('{{ route("eveintelligencecenter::market.groups.save") }}', { _token: '{{ csrf_token() }}', id: group.id, name: newName }, function() { location.reload(); });
    }

    function deleteGroup(id) {
        if (!confirm('Dissolve this group? Fits will not be deleted.')) return;
        $.ajax({
            url: '/eveintelligencecenter/market/groups/' + id,
            method: 'DELETE',
    ...
    function refreshBatchRestock() {
        ...
        $.post('{{ route("eveintelligencecenter::market.fittings.batch-restock") }}', { _token: '{{ csrf_token() }}', fit_ids: ids, percent: pct }, function(res) {
    ...
    function confirmBatchRestock() {
        ...
        $.post('{{ route("eveintelligencecenter::market.exports.save") }}', { _token: '{{ csrf_token() }}', replace_id: replaceId, label: label, export_text: currentBatchText }, function() { location.reload(); });
    }
    ...
    function confirmEditExport() { $.post('{{ route("eveintelligencecenter::market.exports.save") }}', { _token: '{{ csrf_token() }}', replace_id: $('#edit_export_id').val(), label: $('#edit_export_label').val(), export_text: $('#edit_export_text').val() }, function() { location.reload(); }); }
    ...
    function confirmSaveExport() { $.post('{{ route("eveintelligencecenter::market.exports.save") }}', { _token: '{{ csrf_token() }}', replace_id: $('#replace_export_id').val(), label: $('#new_export_label').val(), export_text: $('#export-text').val() }, function() { location.reload(); }); }
    function mergeSelectedExports() { var ids = []; $('.merge-check:checked').each(function() { ids.push($(this).val()); }); if (ids.length == 0) return alert('Select lists first'); $.post('{{ route("eveintelligencecenter::market.exports.dedupe") }}', { _token: '{{ csrf_token() }}', ids: ids }, function(res) { $('#fit-text').val(res.text); $('#fit-paste-form').submit(); }); }
    ...
    function manualSyncHub(hubId) {
        ...
        $.post('{{ route("eveintelligencecenter::market.hubs.sync", ["hubId" => "REPLACEME"]) }}'.replace('REPLACEME', hubId), {
            _token: '{{ csrf_token() }}'
        }, function(res) {
            if (res.success) {
                if ($('#fit_text').val()) {
