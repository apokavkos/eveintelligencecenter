<tr class="fit-row" data-name="{{ strtolower($fit->name) }}" data-label="{{ strtolower($fit->label) }}">
    ...
    <td class="text-right">
        <div class="btn-group">
            <form action="{{ route('eveintelligencecenter::market.fittings') }}" method="POST" style="display:inline;">
                @csrf <input type="hidden" name="fit_text" value="{{ $fit->fit_text }}">
                <button type="submit" class="btn btn-xs btn-info">Load</button>
            </form>
            <form action="{{ route('eveintelligencecenter::market.fittings.delete', $fit->id) }}" method="POST" onsubmit="return confirm('Delete?')" style="display:inline;">
                @csrf @method('DELETE')
