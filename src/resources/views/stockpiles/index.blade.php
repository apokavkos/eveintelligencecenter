                            ...
                            <tbody>
                                @forelse($stockpiles as $stockpile)
                                    <tr class="stockpile-row" style="cursor: pointer;" data-target="#items-{{ $stockpile->id }}">
                                        ...
                                        <td>
                                            <div class="btn-group">
                                                <a href="{{ route('eveintelligencecenter::stockpiles.industry', $stockpile->id) }}" class="btn btn-xs btn-info">
                                                    <i class="fas fa-industry"></i> Industry
                                                </a>
                                                <form action="{{ route('eveintelligencecenter::stockpiles.delete', $stockpile->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this stockpile?')">
                                                    @csrf
                                                    @method('DELETE')
                ...
                <div class="card-body">
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-4 text-right">
                            <div class="btn-group">
                                <a href="{{ route('eveintelligencecenter::stockpiles.workflow') }}" class="btn btn-outline-success">
                                    <i class="fas fa-book-open mr-1"></i> Full Guide
                                </a>
                                <a href="{{ route('eveintelligencecenter::stockpiles.workflow', ['mode' => 'wizard']) }}" class="btn btn-success">
...
<script>
    $(function () {
        $('.location-search').select2({
            ...
            allowClear: true,
            ajax: {
                url: '{{ route("eveintelligencecenter::stockpiles.search.locations") }}',
                dataType: 'json',
                delay: 250,
        ...
        $('.item-location-select').on('change', function() {
            ...
            $.post('{{ route("eveintelligencecenter::stockpiles.item.location", ["itemId" => "REPLACE_ID"]) }}'.replace('REPLACE_ID', itemId), {
                _token: '{{ csrf_token() }}',
